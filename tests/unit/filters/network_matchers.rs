...
mod match_tests {
    ...
    fn is_anchored_by_hostname_works() {
        ...
        assert!(!is_anchored_by_hostname("bar.foo.com", "foo.com", false));
        ...
        assert!(is_anchored_by_hostname("foo.bar", "foo.bar.com", false));
        assert!(is_anchored_by_hostname("foo.bar.", "foo.bar.com", false));
        ...
        // Multiple labels
        assert!(!is_anchored_by_hostname("foo.bar", "foo.barbaz.com", false));
        assert!(!is_anchored_by_hostname(
            ".foo.bar",
            ".foo.barbaz.com",
            false
        ));
        ...
        assert!(!is_anchored_by_hostname("fo.bar.com", "foo.bar.com", false));
        assert!(!is_anchored_by_hostname(
            ".fo.bar.com",
            "foo.bar.com",
            false
        ));
        assert!(!is_anchored_by_hostname("bar.com", "foobar.com", false));
        assert!(!is_anchored_by_hostname(".bar.com", "foobar.com", false));
    ...
    fn check_method_options_works() {
...

        {
            ...
            let request = request::Request::new(url, "", "xhr", "post").unwrap();
            assert!(check_options(&filter, &request));
            let request = request::Request::new(url, "", "xhr", "get").unwrap();
            assert!(!check_options(&filter, &request));
        }
        ...
        {
            let filter = NetworkFilter::parse("||foo", true, Default::default()).unwrap();
            let request = request::Request::new(url, "", "xhr", "post").unwrap();
            assert!(check_options(&filter, &request));
        }

        {
            ...
            let request = request::Request::new(url, "", "xhr", "get").unwrap();
            assert!(!check_options(&filter, &request));
            let request = request::Request::new(url, "", "script", "post").unwrap();
            assert!(!check_options(&filter, &request));
        }
